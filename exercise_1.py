...
for website in website_list:
    ...
    titles = soup1.select('div.col_sub.mainTitle > h1')
    counts=soup1.select('ul.mtit_con_left.fl > li.count')
    dates=soup1.select('ul.mtit_con_left.fl > li.time')
    prices=soup1.select('span.price.c_f50')
    seller_types=soup1.select('li > em.medium')
    areas=soup1.select('span.c_25d > a')
    cateloges=soup1.select('span.crb_i > a')
    print(u'标题：%s,浏览次数：%s,发布日期：%s,价格：%s,区域：%s,卖家类型：%s,类目：%s'%
          (titles,counts,dates,prices,seller_types,areas,cateloges))
